...
// TODO - Redo old style UI with NanoUI. (Port from Baystation)
/obj/machinery/computer/shuttle_control/multi/attack_hand(user as mob)
	if(..(user))
		return
	src.add_fingerprint(user)

	var/datum/shuttle/autodock/multi/MS = SSshuttle.shuttles[shuttle_tag]
	if(!istype(MS))
	...
	if(MS.moving_status != SHUTTLE_IDLE)
		...
		dat += "Location: [MS.get_location_name()]<br>"
	...
	if(MS.skip_docking_checks())
		...
		var/override_en = MS.shuttle_docking_controller?.override_enabled
		var/docking_status = MS.shuttle_docking_controller?.get_docking_status()
...
/obj/machinery/computer/shuttle_control/multi/get_ui_data(var/datum/shuttle/autodock/multi/shuttle)
	...
	if(istype(shuttle))
		. += list(
			...
			"can_cloak" = shuttle.can_cloak,
			"cloaked" = shuttle.cloaked,
		)


//check if we're undocked, give option to force launch
/obj/machinery/computer/shuttle_control/proc/check_docking(datum/shuttle/autodock/MS)
...
/obj/machinery/computer/shuttle_control/multi/Topic(href, href_list)
	...
	var/datum/shuttle/autodock/multi/MS = shuttle_controller.shuttles[shuttle_tag]
	if(!istype(MS)) return
