...
/datum/shuttle
	...
	var/ceiling_type = /turf/unsimulated/floor/shuttle_ceiling // TODO - Document properly
	...
	// TEMP - Stuff that used to be on shuttle, but may not be apropriate here anymore
	// This probably only should be on shuttles that actually DO longjumps?
...
/datum/shuttle/New(_name, var/obj/effect/shuttle_landmark/initial_location)
	...
	//if(logging_home_tag)
	//	new /datum/shuttle_log(src) // TODO - Why not adding to SSshuttle.shuttle_logs? does the datum do that?
...
//
// TODO - Leshana - Docking? Move into subtype? Keep here? We'll see.
// Pro: Moving into subtype IS more object oriented
// Con: Requires us and downstreams to change type paths everywhere.
// Pro: Needing to change type paths everywhere forces people to look closely during upgrade.
//
...
/datum/shuttle/proc/perform_shuttle_move(var/obj/effect/shuttle_landmark/destination, var/list/turf_translation)
	...
	// TODO - Leshana - Uncomment when we have shuttle logs
	// Log shuttle movement if we're a goose and log stuff
	//if(logging_home_tag)
	//	var/datum/shuttle_log/s_log = SSshuttle.shuttle_logs[src]
	//	s_log.handle_move(current_location, destination)

	// Actually do the movement of everything
