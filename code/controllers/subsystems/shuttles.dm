...
// ALSO handles at least something to do with "ships" which I presume is related to the overmap also
// TODO - Leshana investigate "ships" and stuff.  And why that is distinct from shuttles anyway.
//
...
// SUPER TEMP HACK SO I CAN DO DIFFS
var/global/datum/controller/subsystem/shuttles/SSshuttle
...
SUBSYSTEM_DEF(shuttles)
	...
	var/block_queue = TRUE                         // Block initialization of new shuttles/sectors
...
/datum/controller/subsystem/shuttles/PreInit()
	...
	global.SSshuttle = src // TODO - Remove this! Change everything to point at SSshuttles intead
...
/datum/controller/subsystem/shuttles/Initialize(timeofday)
	...
	block_queue = FALSE // TODO - rename to block_init_queues
	clear_init_queue() // TODO - rename to process_init_queues
	return ..()
...
/datum/controller/subsystem/shuttles/proc/clear_init_queue()
	if(block_queue)
		return
	initialize_shuttles()
	initialize_sectors()
...
/datum/controller/subsystem/shuttles/proc/initialize_shuttle(var/shuttle_type)
	...
	if(initial(shuttle.category) != shuttle_type) // Skip if its an "abstract class" datum
		...
		return shuttle
		// TODO - Why not do these things?
		// shuttle.init_docking_controllers() // No need, because shuttle datums fetch refs themselves in New()
			// (Possible because controllers now register *themselves* into the docking_registry)
		// shuttle.dock()  // No need, becasue autodock shuttle datums do this thimselves in New()

// TODO - Leshana Investigate and review this functionality
